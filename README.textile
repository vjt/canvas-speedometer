...
The widget MUST be rendered inside an HTML block element (a `<div>` is fine) from which it inherits the dimensions, set via CSS (`width` and `height`). Positioning is inherited from the container.
...
Initialization follows standard JavaScript object initialization techniques, e.g. `new`. The container name is passed as the first parameter of the constructor, additional parameters are passed using an inline JavaScript object.

Initialization MUST happen after the DOM has been loaded and the CSS parsed, this can be accomplished using any JavaScript framework "onReady" feature (e.g. `$(document).ready()` if you're using jQuery, or `Event.observe('dom:loaded')` if you're using Prototype).
